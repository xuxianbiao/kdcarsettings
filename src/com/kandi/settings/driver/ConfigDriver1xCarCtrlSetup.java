...
public class ConfigDriver1xCarCtrlSetup {
	 ...
	 * 读取车辆模式参数	
	 * @param null																	
	 * @return			包含2个模式，经济模式（0x01）,运动模式（0x02）,暴躁模式（0x03）；
	 */
	//int getCar_WorkMode();
	
	public enum eDrvVMode { 	DRVMODE_SPORT, DRVMODE_ECO, DRVMODE_TEMPER };
	...
	public eDrvVMode getDrvMode() throws RemoteException {
		...
		case 0x03:
			return eDrvVMode.DRVMODE_TEMPER;
		default:
			return eDrvVMode.DRVMODE_SPORT;
		}
	}
	...
	public void setDrvMode(eDrvVMode mode) throws RemoteException {
		...
		case DRVMODE_ECO:
			...
			break;
		case DRVMODE_TEMPER:
			R_service.setCar_WorkMode(0x03);//设置暴躁模式接口使用
			break;
		}
	}
	...
	public String getCarVin(){
		try {
			return R_service.GetCarVin();
		} catch (RemoteException e) {
			return "";
		}
	}
	
	public void requestCarVin(){
		try {
			R_service.requestCarVin();
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	}
